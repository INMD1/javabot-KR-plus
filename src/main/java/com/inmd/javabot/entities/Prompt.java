 ...
 */
package com.inmd.javabot.entities;
...
public class Prompt
{
    ...
    {
        this.title = title;
        this.noguiMessage = noguiMessage == null ? "nogui 모드로 전환 -Dnogui = true 플래그를 포함하여 nogui 모드에서 수동으로 시작할 수 있습니다." : noguiMessage;
        this.nogui = nogui;
    }
    ...
    public String prompt(String content)
    {
        if(nogui)
        {
            ...
            {
                alert(Level.ERROR, title, "명령 행에서 입력을 읽을 수 없습니다.");
                e.printStackTrace();
                return null;
            }
        }
